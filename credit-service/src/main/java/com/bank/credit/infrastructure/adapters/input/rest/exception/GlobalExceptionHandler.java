package com.bank.credit.infrastructure.adapters.input.rest.exception;
...
import com.bank.credit.infrastructure.web.dto.ErrorResponse;
...
public class GlobalExceptionHandler {
  ...
  public Mono<ResponseEntity<ErrorResponse>> handleCreditNotFound(
      CreditNotFoundException ex, ServerWebExchange exchange) {

    LOG.error("handleCreditNotFound: {}", ex.getMessage());
    return Mono.just(
        ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body(buildError("CREDIT_NOT_FOUND", ex.getMessage(), exchange)));
  }
